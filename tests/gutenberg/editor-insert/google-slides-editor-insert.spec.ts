...
const GOOGLE_SLIDES_URL = 'https://docs.google.com/presentation/d/e/2PACX-1vQ3rsCWkulDES2YgcuXvTy36o2n6NDu0nNzbV_YQ3Rpc1fM_BdTyWTEEfzlwdwWPtyrXFvXNsEnAso3/pub?start=true&loop=true&delayms=3000';
const SLIDES_MARKER = '2PACX-1vQ3rsCWkulDES2YgcuXvTy36o2n6NDu0nNzbV';
...
test.describe('Gutenberg editor — Google Slides block insertion (WP 7.0 RC-3)', () => {

  test('EmbedPress block inserts and accepts Google Slides URL', async ({ page }) => {
  ...
  // Google Slides /pub?... URLs are converted by EmbedPress to /embed?... form
  // which Google explicitly supports for iframe embedding. No X-Frame-Options
  // restriction applies from blob: origins on these endpoints.
  test('Google Slides iframe attaches in editor canvas', async ({ page }) => {
    await openNewPost(page);
    await insertEmbedPressBlock(page);
    await fillAndEmbed(page, GOOGLE_SLIDES_URL);

    const canvas = page.frameLocator(CANVAS);
    const slidesIframe = canvas.locator('iframe[src*="docs.google.com/presentation"]').first();
    await expect(slidesIframe).toBeAttached({ timeout: 30_000 });
    await expect(slidesIframe).toHaveAttribute('src', new RegExp(SLIDES_MARKER), { timeout: 5_000 });
  ...
  test('Inspector sidebar loads with General and Custom Branding panels', async ({ page }) => {
    ...
    const canvas = page.frameLocator(CANVAS);
    await expect(canvas.locator('iframe[src*="docs.google.com/presentation"]').first()).toBeAttached({ timeout: 30_000 });
